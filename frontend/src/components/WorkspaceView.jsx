...
export default function WorkspaceView({ workspace, onBack, onContinue, onReload, onDelete, notify, pendingSeek, onSeekHandled }) {
  ...
  return <main className="page workspace-page">
    <header className="workspace-header"><div><button className="back-link" onClick={onBack}>‹ Back to library</button><p className="eyebrow">Lecture workspace</p><input className="title-input" title={title} aria-label="Lecture title" value={title} maxLength="180" onChange={(event) => setTitle(event.target.value)} onBlur={saveTitle} /></div><div className="header-actions">
