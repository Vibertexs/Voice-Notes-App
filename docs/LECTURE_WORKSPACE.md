...
└── Course folder (optional)
    └── Lecture workspace
        ...
        └── Local AI        — generated notes and question history, opt-in
```
...
- `workspace_ai_notes`: editable notes generated by the selected local model,
  kept separate from both student notes and the no-model quick outline.
- `workspace_ai_messages`: a workspace-scoped history of student questions and
...
3. **Study guide** for an editable local review draft made from the student's
   ...
   Its **AI notes** and **Ask this class note** areas use an installed local
   Ollama model when the student opts in; they are disabled with plain setup
...
to an Ollama process at `127.0.0.1`. It constrains each prompt to the current
workspace's student notes and recording transcripts, labels attached but
unparsed files, and saves question history separately. The default setup
